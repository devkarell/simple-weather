...
export function isProvince(acronym: string): string | undefined {
    if (typeof acronym !== 'string' || acronym.length < 2) return;

    const province = provincesMap.find((province) => province.acronym === acronym.toUpperCase());
